/**
 * Self-heal re-confirmation (7th pass) for task mars-5311e0be.
 *
 ...
 * mars-7d1edb09, mars-5b8aace9). This 7th pass runs as self-heal task
 * mars-08c96e5a.
 *
 ...
 *   - git log task/mars-5311e0be..main → 28 commits (was 26 at the 6th
 *     pass; main keeps advancing, including the self-heal trail itself).
 *   - Existing inbox row ecdd51fb open with seen_count=29 prior to this
 *     pass (bumped to 29 by the 6th pass commit 1ae2a89).
 *
 ...
 *     npx tsx scripts/inbox-reconfirm-desync-5311e0be-7th.ts
 */

import { raiseInboxItem } from '../src/mastra/lib/inbox'

const SELF_HEAL_TASK_ID = 'mars-08c96e5a'
...
const main = async (): Promise<void> => {
  const id = await raiseInboxItem({
    ...
    priority: 'high',
    title: `Desync re-confirm (7th pass): task ${TARGET_TASK_ID} landed via rebase under a different SHA`,
    body: [
      `Self-heal task ${SELF_HEAL_TASK_ID} re-confirms (seventh pass) the`,
      ...
      `merge-base ${MERGE_BASE}). Main is now 28 commits ahead.`,
      '',
      ...
      'and stop dispatching repeat self-heal tasks against it. Seven passes',
      'is more than enough signal that the auto-detection rule is missing.',
    ].join('\n'),
    payload: {
      ...
      followUp:
        'Add merged_sha column on tasks (or auto-update task ref post-merge) so the sweeper can detect rebase-landed-but-ref-stale tasks without manual triage. The repeat self-heal dispatches against this row are wasted cycles — seven passes and counting.',
    },
    context: {
      script: 'orchestrator/scripts/inbox-reconfirm-desync-5311e0be-7th.ts',
      precedent: [
        ...
        'commit 1ae2a89 chore(self-heal): re-confirm desync mars-5311e0be stays unhealable (6th pass)',
      ],
      shapeNote:
    ...
    occurrence: {
      ...
      passNumber: 7,
      observation:
        're-confirmed (7th pass): task-branch tip 1d1f8ef still not reachable from main; logical content already shipped as 73921cc; the one-hunk delta matches the post-merge perf refactor e5c3a6a; both prescribed paths still rejected; main now 28 commits ahead.',
    },
  })
