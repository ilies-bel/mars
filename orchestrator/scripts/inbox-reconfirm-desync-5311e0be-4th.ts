/**
 * Self-heal re-confirmation (4th pass) for task mars-5311e0be.
 *
 ...
 * 62cd606), and most-recently c74a52c — see commit log.
 *
 * Confirmed during this fourth self-heal pass (task mars-9fe40cff):
 ...
 *     pass; main has continued advancing — including the prior
 *     1509f97 / 7d1ee88 / d67fbe5 / 62cd606 / c74a52c self-heal trail
 *     for this same desync).
 ...
 *   - Open inbox item ecdd51fb seen_count was 27 before this pass and
 *     has been bumped to 28 by this re-confirmation.
 *
 ...
 * commits (d67fbe5, 7d1ee88, 62cd606, c74a52c) for this same desync.
 *
 * Run from `orchestrator/`:
 *   MARS_REPO=/Users/ib472e5l/project/perso/mars-framework \
 *     npx tsx scripts/inbox-reconfirm-desync-5311e0be-4th.ts
 */

import { raiseInboxItem } from '../src/mastra/lib/inbox'

const SELF_HEAL_TASK_ID = 'mars-9fe40cff'
...
const main = async (): Promise<void> => {
  const id = await raiseInboxItem({
    ...
    priority: 'high',
    title: `Desync re-confirm (4th pass): task ${TARGET_TASK_ID} landed via rebase under a different SHA`,
    body: [
      `Self-heal task ${SELF_HEAL_TASK_ID} re-confirms (fourth pass) the desync`,
      ...
      `merge-base ${MERGE_BASE}). Main is 25 commits ahead (was 18`,
      ...
      'this is the FOURTH self-heal pass against the same row reinforces the',
    ...
    payload: {
      ...
      followUp:
        'Add merged_sha column on tasks (or auto-update task ref post-merge) so the sweeper can detect rebase-landed-but-ref-stale tasks without manual triage. The repeat self-heal dispatches against this row are wasted cycles — this is the 4th pass against the same row.',
    },
    context: {
      script: 'orchestrator/scripts/inbox-reconfirm-desync-5311e0be-4th.ts',
      precedent: [
        ...
        'commit 62cd606 chore(self-heal): re-confirm desync mars-5311e0be stays unhealable (rebase-landed-ref-stale; bumps seen_count on open inbox ecdd51fb to 25, mars-f794be38 pass)',
        'commit c74a52c chore(self-heal): re-confirm desync mars-5311e0be (3rd pass) stays unhealable (rebase-landed-ref-stale; bumps seen_count on open inbox ecdd51fb to 26)',
      ],
      shapeNote:
    ...
    occurrence: {
      ...
      selfHealTaskId: SELF_HEAL_TASK_ID,
      observation:
        're-confirmed (4th pass): task-branch tip 1d1f8ef still not reachable from main; logical content already shipped as 73921cc; the one-hunk delta matches the post-merge perf refactor e5c3a6a; both prescribed paths still rejected. seen_count on inbox ecdd51fb bumped 27→28.',
    },
  })
