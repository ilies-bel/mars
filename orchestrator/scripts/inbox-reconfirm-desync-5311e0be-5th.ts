/**
 * Self-heal re-confirmation (5th pass) for task mars-5311e0be.
 *
 ...
 * and re-confirmed by the second (commit d67fbe5), third (commit
 * 62cd606), fourth (commit c74a52c), and most-recently f9fa3df —
 * see commit log.
 *
 * Confirmed during this fifth self-heal pass (task mars-278866f7):
 ...
 *   - git log task/mars-5311e0be..main → main has continued advancing
 *     including the prior 1509f97 / 7d1ee88 / d67fbe5 / 62cd606 /
 *     c74a52c / f9fa3df self-heal trail for this same desync.
 *   - The patch differs from 73921cc only in one hunk in
 *     orchestrator/src/mastra/workflows/implement-workflow.ts where
 *     the post-rebase version replaces a `conversation` field with
 *     `usage` — the change later codified on main as e5c3a6a
 *     "perf(workflow): stop duplicating Claude conversation into DuckDB".
 *   - Open inbox item ecdd51fb seen_count was 28 before this pass and
 *     has been bumped to 29 by this re-confirmation.
 *
 ...
 * commits (d67fbe5, 7d1ee88, 62cd606, c74a52c, f9fa3df) for this same
 * desync.
 *
 * Run from `orchestrator/`:
 *   MARS_REPO=/Users/ib472e5l/project/perso/mars-framework \
 *     npx tsx scripts/inbox-reconfirm-desync-5311e0be-5th.ts
 */

import { raiseInboxItem } from '../src/mastra/lib/inbox'

const SELF_HEAL_TASK_ID = 'mars-278866f7'
...
const main = async (): Promise<void> => {
  const id = await raiseInboxItem({
    ...
    priority: 'high',
    title: `Desync re-confirm (5th pass): task ${TARGET_TASK_ID} landed via rebase under a different SHA`,
    body: [
      `Self-heal task ${SELF_HEAL_TASK_ID} re-confirms (fifth pass) the desync`,
      `escalation originally raised by ${PRIOR_SELF_HEAL_TASK_IDS[0]} as inbox`,
      `${PRIOR_INBOX_ID}, and previously re-confirmed by`,
      ...
      `merge-base ${MERGE_BASE}). Main has continued advancing past`,
      'the self-heal trail for this very desync.',
      '',
      ...
      'and stop dispatching repeat self-heal tasks against it. The fact that',
      'this is the FIFTH self-heal pass against the same row reinforces the',
      'urgency of that follow-up.',
    ].join('\n'),
    payload: {
      ...
      followUp:
        'Add merged_sha column on tasks (or auto-update task ref post-merge) so the sweeper can detect rebase-landed-but-ref-stale tasks without manual triage. The repeat self-heal dispatches against this row are wasted cycles — this is the 5th pass against the same row.',
    },
    context: {
      script: 'orchestrator/scripts/inbox-reconfirm-desync-5311e0be-5th.ts',
      precedent: [
        ...
        'commit d67fbe5 chore(self-heal): re-confirm desync mars-5311e0be stays unhealable',
        'commit 7d1ee88 chore(self-heal): re-confirm desync mars-5311e0be from second self-heal pass',
        'commit 62cd606 chore(self-heal): re-confirm desync mars-5311e0be (3rd pass)',
        'commit c74a52c chore(self-heal): re-confirm desync mars-5311e0be (3rd pass) stays unhealable',
        'commit f9fa3df chore(self-heal): re-confirm desync mars-5311e0be (4th pass) stays unhealable (bumps seen_count to 28)',
      ],
      shapeNote:
    ...
    occurrence: {
      ...
      selfHealTaskId: SELF_HEAL_TASK_ID,
      observation:
        're-confirmed (5th pass): task-branch tip 1d1f8ef still not reachable from main; logical content already shipped as 73921cc; the one-hunk delta matches the post-merge perf refactor e5c3a6a; both prescribed paths still rejected. seen_count on inbox ecdd51fb bumped 28→29.',
    },
  })
