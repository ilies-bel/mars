/**
 * Self-heal re-confirmation (8th pass) for task mars-5311e0be.
 *
 ...
 * mars-7d1edb09, mars-5b8aace9, mars-08c96e5a). This 8th pass runs as
 * self-heal task mars-e1b7dd50.
 *
 ...
 *   - Existing inbox row ecdd51fb open with seen_count=32 prior to this
 *     pass (bumped by the 7th-pass commit and intervening self-heal sweeps).
 *
 ...
 *     npx tsx scripts/inbox-reconfirm-desync-5311e0be-8th.ts
 */

import { raiseInboxItem } from '../src/mastra/lib/inbox'

const SELF_HEAL_TASK_ID = 'mars-e1b7dd50'
...
const main = async (): Promise<void> => {
  const id = await raiseInboxItem({
    ...
    priority: 'high',
    title: `Desync re-confirm (8th pass): task ${TARGET_TASK_ID} landed via rebase under a different SHA`,
    body: [
      `Self-heal task ${SELF_HEAL_TASK_ID} re-confirms (eighth pass) the`,
      ...
      'and stop dispatching repeat self-heal tasks against it. Eight passes',
    ...
    payload: {
      ...
      followUp:
        'Add merged_sha column on tasks (or auto-update task ref post-merge) so the sweeper can detect rebase-landed-but-ref-stale tasks without manual triage. The repeat self-heal dispatches against this row are wasted cycles — eight passes and counting.',
    },
    context: {
      script: 'orchestrator/scripts/inbox-reconfirm-desync-5311e0be-8th.ts',
      precedent: [
    ...
    occurrence: {
      ...
      passNumber: 8,
      observation:
        're-confirmed (8th pass): task-branch tip 1d1f8ef still not reachable from main; logical content already shipped as 73921cc; the one-hunk delta matches the post-merge perf refactor e5c3a6a; both prescribed paths still rejected; main now 30 commits ahead.',
    },
  })
