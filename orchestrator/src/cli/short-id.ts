 ...
 *   - prefixed:   e.g. `task-2e2bf341`, `prop-09abf133`
 *
 * A naive `id.slice(0, 8)` clips a `task-XXXXXXXX` id down to `task-XXX`,
...
export function shortId(id: string): string {
  ...
  if (dash > 0 && dash < id.length - 1) {
    ...
    // hex chunk — that distinguishes `task-XXXXXXXX` and `prop-XXXXXXXX`
