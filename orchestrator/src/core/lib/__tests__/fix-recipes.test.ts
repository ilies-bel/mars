...
describe('verify:completeness/missing-report recipe', () => {
  const gateOutput =
  ...
  const ctx = {
    targetPath: '/tmp/worktrees/task-mars-819c6152',
    statusOutput: gateOutput,
    targetBranch: 'task/mars-819c6152',
    integrationBranch: 'main',
    originalPrompt: '',
  }

  it('is registered under the correct signature', () => {
    expect(hasRecipe('verify:completeness/missing-report')).toBe(true)
  })

  it('getRecipeOrGeneric returns the purpose-built recipe, not the generic fallback', () => {
    const recipe = getRecipeOrGeneric('verify:completeness/missing-report')
    expect(recipe.signature).toBe('verify:completeness/missing-report')
  })

  it('produces a stable title naming the failing branch', () => {
    const recipe = getRecipe('verify:completeness/missing-report')
    expect(recipe.title(ctx)).toBe(
      'Re-emit completion report for committed work on task/mars-819c6152',
    )
  })

  it('includes the gate output verbatim in the prompt', () => {
    const recipe = getRecipe('verify:completeness/missing-report')
    const prompt = recipe.buildPrompt(ctx)
    expect(prompt).toContain(gateOutput)
  })

  it('instructs the agent to read git log/diff from the failing worktree, not re-implement', () => {
    const recipe = getRecipe('verify:completeness/missing-report')
    const prompt = recipe.buildPrompt(ctx)
    expect(prompt).toContain(`git -C ${ctx.targetPath} log --oneline`)
    expect(prompt).toContain(`git -C ${ctx.targetPath} show --stat HEAD`)
    expect(prompt).toContain(`git -C ${ctx.targetPath} diff HEAD~1..HEAD`)
    expect(prompt).toMatch(/do NOT re-implement/i)
  })

  it('instructs the agent to emit a completion-report block as the fix', () => {
    const recipe = getRecipe('verify:completeness/missing-report')
    const prompt = recipe.buildPrompt(ctx)
    expect(prompt).toContain('```completion-report')
    expect(prompt).toMatch(/\[done\|partial\|blocked\]/i)
    expect(prompt).toMatch(/evidence:/i)
  })

  it('prohibits fabricating evidence', () => {
    const recipe = getRecipe('verify:completeness/missing-report')
    const prompt = recipe.buildPrompt(ctx)
    expect(prompt).toMatch(/do NOT fabricate evidence/i)
  })

  it('instructs the agent NOT to re-run the full verify suite', () => {
    const recipe = getRecipe('verify:completeness/missing-report')
    const prompt = recipe.buildPrompt(ctx)
    expect(prompt).toMatch(/do NOT re-run the full verify suite/i)
  })
  ...
  it('inlines the original task prompt when provided', () => {
    ...
    const promptWithSource = recipe.buildPrompt({
      ...ctx,
      originalPrompt: 'add fix-recipes for completeness failures',
    })
    expect(promptWithSource).toContain('add fix-recipes for completeness failures')
    expect(promptWithSource).toMatch(/inlined/i)
    const promptWithout = recipe.buildPrompt(ctx)
    expect(promptWithout).not.toContain('add fix-recipes for completeness failures')
    expect(promptWithout).not.toMatch(/Original task prompt \(inlined/i)
  })

  it('uses (no gate output captured) fallback when statusOutput is empty', () => {
    const recipe = getRecipe('verify:completeness/missing-report')
    const prompt = recipe.buildPrompt({ ...ctx, statusOutput: '' })
    expect(prompt).toContain('(no gate output captured)')
  })
})

describe('verify:completeness/incomplete recipe', () => {
  const gateOutput =
  ...
  const ctx = {
    targetPath: '/tmp/worktrees/task-mars-incomplete',
    statusOutput: gateOutput,
    targetBranch: 'task/mars-incomplete',
    integrationBranch: 'main',
    originalPrompt: '',
  }

  it('is registered under the correct signature', () => {
    expect(hasRecipe('verify:completeness/incomplete')).toBe(true)
  })

  it('getRecipeOrGeneric returns the purpose-built recipe, not the generic fallback', () => {
    const recipe = getRecipeOrGeneric('verify:completeness/incomplete')
    expect(recipe.signature).toBe('verify:completeness/incomplete')
  })

  it('produces a stable title naming the failing branch', () => {
    const recipe = getRecipe('verify:completeness/incomplete')
    expect(recipe.title(ctx)).toBe(
      'Finish the listed unmet criteria and update the completion report on task/mars-incomplete',
    )
  })

  it('includes the gate output verbatim in the prompt so the recovery sees the exact unmet criteria', () => {
    const recipe = getRecipe('verify:completeness/incomplete')
    const prompt = recipe.buildPrompt(ctx)
    expect(prompt).toContain(gateOutput)
  })

  it('instructs the agent to finish ONLY the listed unmet criteria and not touch already-done work', () => {
    const recipe = getRecipe('verify:completeness/incomplete')
    const prompt = recipe.buildPrompt(ctx)
    expect(prompt).toMatch(/ONLY the criteria.*listed as.*partial.*blocked/i)
    expect(prompt).toMatch(/already done|already marked.*done/i)
    expect(prompt).toMatch(/do NOT re-implement/i)
  })

  it('includes a lift-diff step to bring committed work into the recovery worktree', () => {
    const recipe = getRecipe('verify:completeness/incomplete')
    const prompt = recipe.buildPrompt(ctx)
    expect(prompt).toContain(`git -C ${ctx.targetPath} diff`)
    expect(prompt).toContain('$BASE..HEAD')
    expect(prompt).toContain('git apply --3way')
    expect(prompt).toMatch(/commit.*lift/i)
  })

  it('instructs the agent to emit a corrected completion-report block', () => {
    const recipe = getRecipe('verify:completeness/incomplete')
    const prompt = recipe.buildPrompt(ctx)
    expect(prompt).toContain('```completion-report')
    expect(prompt).toMatch(/\[done\]/i)
  })

  it('embeds the failing branch, worktree path, and integration branch', () => {
    const recipe = getRecipe('verify:completeness/incomplete')
    const prompt = recipe.buildPrompt(ctx)
    expect(prompt).toContain(ctx.targetBranch)
    expect(prompt).toContain(ctx.targetPath)
...
describe('verify:completeness/unsubstantiated recipe', () => {
  const gateOutput =
  ...
  const ctx = {
    targetPath: '/tmp/worktrees/task-mars-unsub',
    statusOutput: gateOutput,
    targetBranch: 'task/mars-unsub',
    integrationBranch: 'main',
    originalPrompt: '',
  }

  it('is registered under the correct signature', () => {
    expect(hasRecipe('verify:completeness/unsubstantiated')).toBe(true)
  })

  it('getRecipeOrGeneric returns the purpose-built recipe, not the generic fallback', () => {
    const recipe = getRecipeOrGeneric('verify:completeness/unsubstantiated')
    expect(recipe.signature).toBe('verify:completeness/unsubstantiated')
  })

  it('produces a stable title naming the failing branch', () => {
    const recipe = getRecipe('verify:completeness/unsubstantiated')
    expect(recipe.title(ctx)).toBe(
      'Provide verifiable evidence for completion claims on task/mars-unsub',
    )
  })

  it('includes the gate output verbatim in the prompt so the recovery sees the exact failing claims', () => {
    const recipe = getRecipe('verify:completeness/unsubstantiated')
    const prompt = recipe.buildPrompt(ctx)
    expect(prompt).toContain(gateOutput)
  })

  it('instructs the agent to fix evidence references, not re-implement the task', () => {
    const recipe = getRecipe('verify:completeness/unsubstantiated')
    const prompt = recipe.buildPrompt(ctx)
    expect(prompt).toMatch(/do NOT re-implement the task/i)
    expect(prompt).toMatch(/do NOT re-run the full verify suite/i)
    expect(prompt).toMatch(/only fix the failing claims/i)
  })
  ...
  it('instructs the agent to look up real evidence from git log/diff', () => {
    const recipe = getRecipe('verify:completeness/unsubstantiated')
    const prompt = recipe.buildPrompt(ctx)
    expect(prompt).toContain(`git -C ${ctx.targetPath} log --oneline`)
    expect(prompt).toContain(`git -C ${ctx.targetPath} show --stat HEAD`)
...
describe('all three completeness recipes resolve to scoped (non-generic) handlers', () => {
  // Regression guard: before this fix, all three produced verify:completeness/unclassified
  // which routed to genericRecoveryRecipe — a first-principles recovery that re-read the
  // whole task, re-explored the tree, and re-verified everything. These assertions confirm
  // each signature dispatches to its own purpose-built recipe.
  it('verify:completeness/missing-report has a registered recipe (not the generic fallback)', () => {
    expect(hasRecipe('verify:completeness/missing-report')).toBe(true)
    const recipe = getRecipeOrGeneric('verify:completeness/missing-report')
    expect(recipe.signature).not.toBe('generic/no-recipe')
  })

  it('verify:completeness/incomplete has a registered recipe (not the generic fallback)', () => {
    expect(hasRecipe('verify:completeness/incomplete')).toBe(true)
    const recipe = getRecipeOrGeneric('verify:completeness/incomplete')
    expect(recipe.signature).not.toBe('generic/no-recipe')
  })

  it('verify:completeness/unsubstantiated has a registered recipe (not the generic fallback)', () => {
    expect(hasRecipe('verify:completeness/unsubstantiated')).toBe(true)
    const recipe = getRecipeOrGeneric('verify:completeness/unsubstantiated')
    expect(recipe.signature).not.toBe('generic/no-recipe')
  })
