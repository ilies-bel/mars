...
describe('worktree-install', () => {
  ...
  describe('installWorktreeDeps', () => {
    ...
    it('installs then builds a local file: workspace dep BEFORE installing the site (so dist is packed)', async () => {
      ...
      // The workflow dir is itself a detected site AND a build target. We care
      // about the orchestrator-driven build sequence: dep install -> dep build
      // -> orchestrator install. Scope to calls whose cwd is the workflow dir
      // (the dep install + build) vs the orchestrator install.
    ...
    it('does not build deps that escape the worktree or lack a build script', async () => {
      ...
      writeFileSync(
        ...
        JSON.stringify({
          name: 'orch',
          dependencies: {
            // registry dep — ignored
            zod: '^4.0.0',
            // file: dep escaping the worktree — ignored
            outside: 'file:../../elsewhere',
          },
        }),
      )
      const calls: RecordedCall[] = []
      ...
      await installWorktreeDeps({ worktreeRoot: workDir, runner })
      // Only the install runs; no build.
