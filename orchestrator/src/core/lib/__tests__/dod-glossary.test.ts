 ...
 * registered in the project glossary (CONTEXT.md).
 *
...
import { readGlossaryFile } from '../glossary'
...
describe('DoD vocabulary in project glossary', () => {
  it('glossary has a Tag entry describing an author-supplied label on a Task that materialises criteria', async () => {
    const doc = await readGlossaryFile(CONTEXT_PATH)
    const term = doc.terms.find((t) => t.term.toLowerCase() === 'tag')
    expect(term, 'Tag term should be registered in CONTEXT.md').toBeDefined()
  ...
  it('glossary has a Definition of Done entry describing criteria a Task must satisfy before verify passes', async () => {
    const doc = await readGlossaryFile(CONTEXT_PATH)
    const term = doc.terms.find((t) => t.term.toLowerCase() === 'definition of done')
    expect(term, '"Definition of Done" term should be registered in CONTEXT.md').toBeDefined()
    expect(term!.definition.toLowerCase()).toMatch(/criteria/)
  ...
  it('glossary has a Criterion entry describing a single free-text outcome the agent must validate or waive', async () => {
    const doc = await readGlossaryFile(CONTEXT_PATH)
    const term = doc.terms.find((t) => t.term.toLowerCase() === 'criterion')
    expect(term, 'Criterion term should be registered in CONTEXT.md').toBeDefined()
  ...
  it('glossary has a validate entry describing the agent verb for marking a criterion satisfied', async () => {
    const doc = await readGlossaryFile(CONTEXT_PATH)
    const term = doc.terms.find((t) => t.term.toLowerCase() === 'validate')
    expect(term, 'validate term should be registered in CONTEXT.md').toBeDefined()
  ...
  it('glossary has a waive entry describing the agent verb for skipping a criterion with a recorded reason', async () => {
    const doc = await readGlossaryFile(CONTEXT_PATH)
    const term = doc.terms.find((t) => t.term.toLowerCase() === 'waive')
    expect(term, 'waive term should be registered in CONTEXT.md').toBeDefined()
    expect(term!.definition.toLowerCase()).toMatch(/criterion/i)
