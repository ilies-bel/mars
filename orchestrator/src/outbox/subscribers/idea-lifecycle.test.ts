...
describe('idea-lifecycle outbox subscriber', () => {
  ...
  it('processes a proposal.added event — exactly one event per transition', async () => {
    ...
    await sub.ensureIdeaLifecycleSubscriber(q.resolveQueueClient())
    await pub.publishWithRetry(q.resolveQueueClient(), 'proposal.added', {
      proposalId: 'prop-1',
      source: 'human',
      title: 'My idea',
    })

    const { processed } = await sub.drainIdeaLifecycle(q.resolveQueueClient())
  ...
  it('processes proposal.promoted, proposal.dismissed, and proposal.deleted transitions', async () => {
    ...
    await pub.publishWithRetry(q.resolveQueueClient(), 'proposal.promoted', { proposalId: 'prop-p' })
    await pub.publishWithRetry(q.resolveQueueClient(), 'proposal.dismissed', { proposalId: 'prop-d' })
    await pub.publishWithRetry(q.resolveQueueClient(), 'proposal.deleted', { proposalId: 'prop-del' })
  ...
  it('ignores non-lifecycle proposal events (e.g. proposal.sliced)', async () => {
    ...
    await pub.publishWithRetry(q.resolveQueueClient(), 'proposal.sliced', {
      proposalId: 'prop-s',
      taskCount: 3,
    })
  ...
  it('fires exactly once per transition — draining the same event twice is idempotent', async () => {
    ...
    const first = await sub.drainIdeaLifecycle(q.resolveQueueClient())
    const second = await sub.drainIdeaLifecycle(q.resolveQueueClient())
  ...
  it('mixed bag: counts only lifecycle events, ignores others in the same drain', async () => {
    ...
    await pub.publishWithRetry(q.resolveQueueClient(), 'proposal.promoted', {
      proposalId: 'prop-m1',
    })
