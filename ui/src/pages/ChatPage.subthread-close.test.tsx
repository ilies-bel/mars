...
const { createSubthreadAndSend, fetchChatThread } = vi.hoisted(() => ({
...
vi.mock('@/shared/api', () => ({
  ...
  createChatThread: vi.fn(), createSubthreadAndSend, endChatSubthread: vi.fn(), postChatMessage: vi.fn(), uploadAttachment: vi.fn(),
...
beforeEach(() => {
  ...
  createSubthreadAndSend.mockResolvedValue({ id: 'subthread-1', title: '', status: 'idle', attentionStatus: 'idle', createdAt: '', updatedAt: '', origin: null, alertItemId: null, alertResolved: false, closedAt: null, terminalEventType: null, parentThreadId: null })
  fetchChatThread.mockResolvedValue({
    thread: { id: 'subthread-1', title: '', status: 'idle', attentionStatus: 'idle', createdAt: '', updatedAt: '', origin: null, alertItemId: null, alertResolved: false, closedAt: '2026-08-01T00:00:00.000Z', terminalEventType: null, parentThreadId: null },
    messages: [],
  })
...
describe('ChatPage Subthread closure', () => {
  it('returns an automatically closed active Subthread to the boundary composer', async () => {
    await act(async () => {
    ...
    await act(async () => {
      Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype, 'value')?.set?.call(input, 'Finish this Subthread')
      input.dispatchEvent(new Event('input', { bubbles: true }))
      input.dispatchEvent(new Event('change', { bubbles: true }))
      ;(container.querySelector('[data-testid="hero-send"]') as HTMLButtonElement).click()
    })

    await vi.waitFor(() => expect(fetchChatThread).toHaveBeenCalledWith('subthread-1', undefined))
    await vi.waitFor(() => expect(container.querySelector('[data-testid="active-subthread"]')).toBeNull())
