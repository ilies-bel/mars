    ...
    <div className="flex w-full flex-col gap-[10px] rounded-lg border border-border bg-surface p-[18px]">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {allowed.map((tool) => (
          <span
            key={`allowed-${tool}`}
            className="rounded border border-border bg-neutral-100 px-2 py-0.5 font-mono text-meta text-fg"
          >
            {tool}
          </span>
        ))}
        {denied.map((tool) => (
          <span
            key={`denied-${tool}`}
            className="rounded bg-iron/10 px-2 py-0.5 font-mono text-meta text-iron"
          >
