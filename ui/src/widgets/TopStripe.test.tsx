...
describe('TopStripe – stat labels match their values', () => {
  ...
  it('shows the DONE TODAY count', () => {
    const html = renderToStaticMarkup(
      <TopStripe inProgress={3} doneToday={5} failed={2} connected={true} />,
    )
    expect(html).toContain('5 DONE TODAY')
  })
  ...
  it('DONE TODAY and FAILED counts are independent — different values render as distinct stats', () => {
    ...
    expect(html).toContain('7 DONE TODAY')
    expect(html).toContain('4 FAILED')
    // The FAILED count (4) must not bleed into the DONE TODAY slot
    expect(html).not.toContain('4 DONE TODAY')
    // The DONE TODAY count (7) must not bleed into the FAILED slot
