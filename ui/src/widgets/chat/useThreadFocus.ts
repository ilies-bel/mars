 ...
 *   4. For task-linked kinds (failed-task, arc-failed, task-blocked) → cross-
...
import type { ActionQueueItem, ChatThread, ProgressTask } from '@/shared/schemas'

/** Action queue item kinds that correspond to a tracked ProgressTask entity. */
const TASK_LINKED_KINDS = new Set(['failed-task', 'arc-failed', 'task-blocked'])
...
export const useThreadFocus = (thread?: ChatThread): ThreadFocusResult => {
  ...
  if (TASK_LINKED_KINDS.has(item.kind)) {
