...
## Goal
Turn a single user goal into a `Plan` of independently-executable tasks with explicit dependencies and acceptance criteria.

## Definition of Done
- Output is a valid `Plan` (per `docs/CONTRACTS.md` §4) written to `intent.json`.
...
- The plan covers the goal with no implicit gaps that would force the builder to ask a clarifying question.
